...
apt-get install -y git dkms curl dwarves linux-headers-`uname -r` linux-source-6.1
...
echo "EXTRA_CFLAGS=-I\$(PWD)/include -D_MAIN_KERNEL_ -DCONFIG_DXGKRNL=m -include /usr/src/dxgkrnl-$VERSION/include/extra-defines.h -I /usr/src/dxgkrnl-$VERSION/include/libdxg/ -I /usr/src/linux-headers-6.1.0-30-common/include/linux/ -include /usr/src/linux-headers-6.1.0-30-common/include/linux/vmalloc.h -include /usr/src/dxgkrnl-$VERSION/include/uapi/misc/d3dkmthk.h  -Wno-empty-body" >> /usr/src/dxgkrnl-$VERSION/Makefile
